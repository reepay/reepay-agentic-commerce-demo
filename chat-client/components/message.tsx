...
const PurePreviewMessage = ({
  ...
  // Check if this message is a tool-trigger message (should be hidden from UI)
  const isToolTriggerMessage = message.annotations?.some(
    (annotation: any) => annotation.type === 'tool-trigger'
  );

  // Don't render tool-trigger messages
  if (isToolTriggerMessage) {
    return null;
  }

  const shouldShowCopyButton = message.role === "assistant" && (!isLatestMessage || status !== "streaming");

  // Separate parts into UI resources (to render at end) and everything else
  const regularParts: JSX.Element[] = [];
  const uiResourceParts: JSX.Element[] = [];

  message.parts?.forEach((part, i) => {
    let element: JSX.Element | null = null;
    let isUiResource = false;
    ...
    switch (part.type) {
      ...
      case "tool-invocation":
        ...
        const isToolHidden = hiddenTools?.has(toolKey) || false;

        // Check if this tool invocation has UI resources
        let hasUiResources = false;
        if (result && typeof result === 'object') {
          const content = result.content || (typeof result === 'string' ? JSON.parse(result).content : null);
          hasUiResources = content?.some((item: any) =>
            item.type === 'resource' && item.resource?.uri?.startsWith('ui://')
          );
        } else if (typeof result === 'string') {
          try {
            const parsed = JSON.parse(result);
            hasUiResources = parsed.content?.some((item: any) =>
              item.type === 'resource' && item.resource?.uri?.startsWith('ui://')
            );
          } catch {}
        }

        isUiResource = hasUiResources;
    ...
    if (element) {
      if (isUiResource) {
        uiResourceParts.push(element);
      } else {
        regularParts.push(element);
      }
    }
  });
    ...
    >
      ...
      >
        <div className="flex flex-col w-full space-y-3">
          {/* Render regular parts first */}
          {regularParts}

          {/* Then render UI resource parts at the end */}
          {uiResourceParts}
...
export const Message = memo(PurePreviewMessage, (prevProps, nextProps) => {
  ...
  if (prevProps.hiddenTools !== nextProps.hiddenTools) return false;
  if (prevProps.onToolInvoked !== nextProps.onToolInvoked) return false;
